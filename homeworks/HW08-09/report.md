...
- **O1:** LR слишком большой — Adam, lr=0.1, 8 эпох. Accuracy ≈ 0.16 (уровень случайного угадывания).
- **O2:** LR слишком маленький — Adam, lr=1e-5, 8 эпох. Accuracy ≈ 0.77 (обучение почти не движется).
- **O3:** SGD, momentum=0.9, weight_decay=1e-4, lr=1e-2, 15 эпох. Val accuracy ≈ 0.95 — сопоставимо с Adam.
...
- O1 (слишком большой LR): loss нестабилен, accuracy на уровне случайного угадывания (~0.16).
- O2 (слишком маленький LR): метрики почти не меняются за 8 эпох, обучение крайне медленное.
- O3 (SGD+momentum+weight decay): по метрикам близко к Adam, сопоставимое качество.
...
O1 при слишком большом LR (0.1) д — loss скачет, accuracy остаётся на уровне случайного угадывания. O2 при слишком маленьком LR (1e-5) почти не двигает loss и accuracy за 8 эпох — обучение слишком медленное. SGD+momentum с weight decay (O3) показывает, что при правильном LR можно достичь сопоставимого качества с Adam, а weight decay обеспечивает дополнительную регуляризацию весов.
