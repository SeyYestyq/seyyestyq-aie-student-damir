...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame, limit_unique: int = 55) -> Dict[str, Any]:
    """
    ...
    has_suspicious_id_dublictes = False
    for col in summary.columns:
        if col.name == "user_id":
            if col.unique < col.non_null:
                has_suspicious_id_dublictes = True
                break
    flags["has_suspicious_id_dublictes"] = has_suspicious_id_dublictes

    has_high_cardinality_categoricals = False
    for col in summary.columns:
        if not col.is_numeric:
            if (col.unique/col.non_null)*100 > limit_unique:
    ...
    if summary.n_cols > 100:
        score -= 0.1
    if has_suspicious_id_dublictes:
        score -= 0.4
